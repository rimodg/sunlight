...
# ═══════════════════════════════════════════════════════════
# SECTION 4: RULE BUILDER — Closure pattern for profile params
# ═══════════════════════════════════════════════════════════

def build_rules(profile: JurisdictionProfile) -> List[Rule]:
    """
    Build all 16 TCA rules with jurisdiction-specific parameters.

    Uses closure pattern: rules' lambdas close over the profile parameter,
    binding them to the profile at construction time. This avoids modifying
    the Rule dataclass signature while allowing profile parameterization.
    ...
    rules.append(Rule(
        ...
        evidence=proc_001_evidence,
        condition=lambda f: (
            f["procurement_method"] in ("direct", "limited", "sole_source")
            and f["tender_value"] > profile.competitive_threshold
        ),
        nodes=lambda f: [],
        edges=lambda f: [{
            "source": "award", "target": "process", "type": "REMOVES", "weight": 0.9,
            "rule": "PROC-001",
            "description": f"Direct award of {f['currency']} {f['tender_value']:,.0f} exceeds competitive threshold ({f['currency']} {profile.competitive_threshold:,.0f})"
        }],
    ))
    ...
    # Build evidence string for ENT-001 from profile
    ent_001_evidence = profile.legal_citations.get("case_authority",
        "DOJ US v. Marquez (Maryland 2024): two companies at 7100 Columbia Gateway Dr submitted coordinated bids")
    ...
    # Build evidence string for ENT-002 from profile
    ent_002_evidence = profile.legal_citations.get("case_authority",
        "DOJ US v. Marquez: Marquez IT Co 1 and Marquez IT Co 2 owned by same individual")
    ...
    rules.append(Rule(
        ...
        condition=lambda f: (
            ...
            and f["award_value"] > 0
            and f["tender_value"] != f["award_value"]
            and (1 - profile.competitive_pricing_tolerance_pct / 100) <= f["award_value"] / f["tender_value"] <= (1 + profile.competitive_pricing_tolerance_pct / 100)
        ),
        nodes=lambda f: [],
        edges=lambda f: [{
            "source": "award", "target": "budget",
    ...
    rules.append(Rule(
        ...
        condition=lambda f: (
            ...
            and f["award_month"] == profile.fiscal_year_end_month
            and f["award_day"] >= (profile.fiscal_year_end_day - 15)
        ),
    ...
    rules.append(Rule(
        ...
        condition=lambda f: (
            f["award_month"] is not None
            and f["award_month"] in profile.fiscal_q4_months
    ...
    rules.append(Rule(
        ...
        condition=lambda f: (
            f["award_month"] is not None
            and f["award_month"] in profile.fiscal_safe_months
        ),
        nodes=lambda f: [],
        edges=lambda f: [{
            "source": "award", "target": "budget",
...
# ═══════════════════════════════════════════════════════════
# SECTION 5: MODULE-LEVEL RULES (backwards compatibility)
# ═══════════════════════════════════════════════════════════

RULES: List[Rule] = build_rules(US_FEDERAL)
...
class TCAGraphRuleEngine:
    ...
    def __init__(self, rules: Optional[List[Rule]] = None, profile: Optional[JurisdictionProfile] = None):
        if rules is not None:
            self.rules = rules
        elif profile is not None:
            self.rules = build_rules(profile)
        else:
            self.rules = RULES  # default: build_rules(US_FEDERAL)
...
# ═══════════════════════════════════════════════════════════
# SECTION 7: ADAPTER — Protocol interface for sunlight_core.py
# ═══════════════════════════════════════════════════════════
...
class TCAGraphRuleEngineAdapter:
    ...
    def __init__(self, profile: Optional[JurisdictionProfile] = None):
        self.engine = TCAGraphRuleEngine(profile=profile)
